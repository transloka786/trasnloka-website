...
// Source recording preserved; its native media timeline skips the quiet first 10s.
...
export default function JourneyExperience({children}:{children:ReactNode}){
 ...
 useEffect(()=>{
  ...
  let disposed=false,raf=0,last=0,meterPending=false,scrollTimer:ReturnType<typeof setTimeout>|undefined,seekTimer:ReturnType<typeof setTimeout>|undefined;
  ...
  const attachMeter=async()=>{
   // Touch devices use native media output. The decorative meter must never silence their soundtrack.
   if(matchMedia('(any-pointer:coarse),(hover:none)').matches)return;
   if(ctx){void ctx.resume().catch(()=>{});return;}if(meterPending||disposed)return;meterPending=true;
   try{const candidate=new AudioContext();await candidate.resume();if(disposed||candidate.state!=='running'){await candidate.close();return;}const node=candidate.createAnalyser();node.fftSize=512;const source=candidate.createMediaElementSource(a);source.connect(node);node.connect(candidate.destination);ctx=candidate;analyser=node;}
   catch{/* Native HTML audio is the fallback. */}finally{meterPending=false;}
  };
  const seekPastIntro=()=>{
   // Setting the default playback position is allowed before metadata in supporting browsers.
   // A #t=10 media fragment and the metadata handler cover browsers that defer the seek.
   try{if(a.currentTime<START)a.currentTime=Math.min(START,Number.isFinite(a.duration)?a.duration:END);}catch{}
  };
  const attempt=()=>{
   const s=state.current;if(disposed||document.hidden||!s.available||s.muted||s.ended||s.attempting)return;
   a.muted=false;seekPastIntro();s.attempting=true;
   // Do not gate play() on readyState: iOS may load metadata only after a trusted play call.
  ...
  const scroll=()=>{if(state.current.mode==='guided')return;if(scrollTimer)clearTimeout(scrollTimer);scrollTimer=setTimeout(()=>{
   ...
   if(index!==s.chapter){s.chapter=index;s.ended=false;if(s.available){if(seekTimer)clearTimeout(seekTimer);a.volume=.08;seekTimer=setTimeout(()=>{if(disposed)return;try{a.currentTime=Math.min(CHAPTERS[index].start,a.duration||END);}catch{}a.volume=.36;attempt();},120);}}
  },180);};
  ...
  void fetch('/audio/score.json',{signal:abort.signal,cache:'no-store'}).then(r=>r.ok?r.json():null).then((asset:{available?:boolean;src?:string}|null)=>{
   ...
   state.current.available=true;a.src=asset.src+'#t='+START;a.load();attempt();mark();
  }).catch(()=>{if(!disposed)mark();});
  const gestures=['pointerup','touchend','click','keydown'];
  for(const name of gestures)window.addEventListener(name,gesture,{passive:true});
  window.addEventListener('wheel',user,{passive:true});window.addEventListener('touchmove',user,{passive:true});window.addEventListener('scroll',scroll,{passive:true});window.addEventListener('resize',measure);document.addEventListener('visibilitychange',visibility);a.addEventListener('ended',ended);a.addEventListener('error',error);raf=requestAnimationFrame(tick);
  return()=>{disposed=true;abort.abort();cancelAnimationFrame(raf);ro.disconnect();if(scrollTimer)clearTimeout(scrollTimer);if(seekTimer)clearTimeout(seekTimer);for(const name of gestures)window.removeEventListener(name,gesture);window.removeEventListener('wheel',user);window.removeEventListener('touchmove',user);window.removeEventListener('scroll',scroll);window.removeEventListener('resize',measure);document.removeEventListener('visibilitychange',visibility);a.removeEventListener('loadedmetadata',metadata);a.removeEventListener('ended',ended);a.removeEventListener('error',error);a.pause();a.removeAttribute('src');a.load();void ctx?.close().catch(()=>{});};
 },[]);
