...
/** Desktop displacement; touch/mobile content never depends on a raster mask. */
export default function MosaicReveal({root,replay=0}:{root:RefObject<HTMLDivElement|null>;replay?:number}){
 ...
 useEffect(()=>{
  ...
  const clearContentEffects=()=>{for(const key of ['mask-image','-webkit-mask-image','mask-size','-webkit-mask-size','filter','-webkit-filter'])element.style.removeProperty(key);};
  const clear=()=>{clearContentEffects();element.dataset.reveal='ready';hero.dataset.mosaic='off';};
  ...
  const handheld=matchMedia('(max-width: 1024px), (any-pointer: coarse), (hover: none)');
  const iosWebKit=CSS.supports('-webkit-touch-callout','none');
  let contentEffects=false,width=0,height=0,contentWidth=0,contentHeight=0,cols=0,rows=0,pitch=18,offsetX=0,offsetY=0;
  ...
  let pointer={x:-999,y:-999},energy=0;
  const smooth=(v:number)=>{v=Math.max(0,Math.min(1,v));return v*v*(3-2*v);};
  const resize=()=>{
   contentEffects=!handheld.matches&&!iosWebKit;
   hero.dataset.mosaicContent=contentEffects?'desktop-filter':'unmasked';
   if(!contentEffects){clearContentEffects();element.dataset.reveal='ready';}
   ...
   if(nextW===width&&nextH===height&&content.width===contentWidth&&content.height===contentHeight)return;
   width=nextW;height=nextH;contentWidth=content.width;contentHeight=content.height;pitch=innerWidth<600?16:18;
   const dpr=Math.min(devicePixelRatio||1,1.5);
   field.width=Math.round(width*dpr);field.height=Math.round(height*dpr);context.setTransform(dpr,0,0,dpr,0,0);
   cols=Math.max(1,Math.ceil(content.width/pitch));rows=Math.max(1,Math.ceil(content.height/pitch));
  ...
  const draw=(now:number)=>{
   ...
   // Real visible elapsed time: slow GPUs must not stretch a 2.2s reveal indefinitely.
   const dt=last?Math.max(0,(now-last)/1000):0;last=now;elapsed+=dt;
   ...
   if(contentEffects){alpha.clearRect(0,0,mask.width,mask.height);vector.fillStyle='rgb(128,128,128)';vector.fillRect(0,0,map.width,map.height);}
   ...
   for(let y=0;y<rows;y++)for(let x=0;x<cols;x++){
    ...
    if(contentEffects){
     const a=intro*(isCopy?.975+.025*breathe:.86+.12*breathe),gap=(isCopy?.025:.13)+(1-intro)*.75;
     alpha.fillStyle=`rgba(255,255,255,${a})`;alpha.fillRect(x*4+gap,y*4+gap,4-gap*2,4-gap*2);
     const dx=(Math.sin(phase)*.16+ripple*.66+cross*.07)*readability,dy=(Math.cos(phase*.8)*.18+ripple*.76)*readability;
   ...
   if(contentEffects){
    const texture=`url(${mask.toDataURL('image/png')})`;
    image.current?.setAttribute('href',map.toDataURL('image/png'));displace.current?.setAttribute('scale','9');
    element.style.setProperty('mask-size','100% 100%');element.style.setProperty('-webkit-mask-size','100% 100%');
    element.style.setProperty('mask-image',texture);element.style.setProperty('-webkit-mask-image',texture);element.style.filter=`url(#${id})`;
   }
   element.dataset.reveal=contentEffects&&reveal<1?'running':'ready';hero.dataset.mosaic=reveal<1?'revealing':'pulsing';hero.dataset.mosaicFrame=String(++frames);
  };
  ...
  const move=(event:PointerEvent)=>{
   ...
   const speed=Math.min(1,Math.hypot(x-pointer.x,y-pointer.y)/45);pointer={x,y};
  ...
  const ro=new ResizeObserver(resize);ro.observe(hero);ro.observe(element);handheld.addEventListener('change',resize);
  ...
  return()=>{disposed=true;cancelAnimationFrame(frame);observer.disconnect();ro.disconnect();handheld.removeEventListener('change',resize);hero.removeEventListener('pointermove',move);hero.removeEventListener('pointerdown',move);document.removeEventListener('visibilitychange',visibility);context.clearRect(0,0,width,height);clear();};
