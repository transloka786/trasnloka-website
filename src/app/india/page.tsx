...
import { IndiaEvidence } from '@/components/EvidenceModules';
...
const PILLARS = [
  { title: 'Disease relevance', text: 'KritRNA begins with selected nonsense-mutation contexts in HBB, DMD and TP53, including an India-priority focus on β-thalassemia.', accent: 'var(--magenta)' },
  { title: 'Scientific ownership', text: 'The computational platform, candidate logic and future experimental evidence are being organised as company-owned capability developed from India.', accent: 'var(--cyan)' },
  { title: 'Validation network', text: 'The development path depends on Indian CROs, research laboratories, clinicians, hospitals and rare-disease communities working through formal collaborations.', accent: 'var(--forest)' },
  { title: 'Access-aware development', text: 'Early decisions about indication, assay, delivery and evidence should reflect the realities of patients, healthcare systems and research infrastructure in India.', accent: 'var(--violet)' },
];
  ...
  return (
    <>
      ...
      <section className="ink-sec"><div className="wrap">
        ...
        <div className="grid cols-4" style={{ background: 'rgba(244,234,213,.14)', borderColor: 'rgba(244,234,213,.14)' }}>{PILLARS.map((pillar, i) => <Reveal key={pillar.title} delay={i * 0.05}><article className="cell" style={{ height: '100%', background: i % 2 ? '#141210' : '#10100f', borderTop: `4px solid ${pillar.accent}` }}><h3 style={{ color: 'var(--cream)' }}>{pillar.title}</h3><p style={{ color: 'rgba(244,234,213,.7)' }}>{pillar.text}</p></article></Reveal>)}</div>
      </div></section>

      <IndiaEvidence />

      <section><div className="wrap split-2" style={{ display: 'grid', gap: 46, alignItems: 'start' }}>
        <Reveal><div><div className="eyebrow">Initial focus</div><h2 className="h2" style={{ marginTop: 14 }}>Three programs with different biological and access questions</h2><p className="lead" style={{ marginTop: 20 }}>HBB, DMD and TP53 provide distinct tests of the same platform thesis. Each program requires its own mutation context, amino-acid restoration logic, assay system, delivery strategy and safety evidence.</p><div className="callout" style={{ marginTop: 24 }}>These are early research programs. KritRNA does not currently offer a treatment, clinical trial or patient-enrolment pathway.</div></div></Reveal>
        <Reveal delay={0.08}><div style={{ border: '1px solid var(--rule)', borderRadius: 10, background: 'var(--paper)', overflow: 'hidden' }}>{[
          ...
          ['TP53 / p53 cancer biology', 'Platform-breadth oncology research focused on selected nonsense contexts and functional restoration questions.'],
        ].map(([title, text], i) => <div key={title} style={{ padding: '21px 24px', borderTop: i ? '1px solid var(--rule)' : 'none' }}><div style={{ fontFamily: 'var(--mono)', fontSize: '.68rem', color: i === 0 ? 'var(--magenta)' : i === 1 ? 'var(--cyan)' : 'var(--violet)' }}>0{i + 1}</div><h3 style={{ marginTop: 6 }}>{title}</h3><p style={{ color: 'var(--ink-soft)', marginTop: 7 }}>{text}</p></div>)}</div></Reveal>
      </div></section>
      ...
      <section><div className="wrap">
        ...
        <div className="grid cols-2">{PATH.map(([number, title, text], i) => <Reveal key={number} delay={(i % 2) * 0.05}><article className="cell" style={{ height: '100%', display: 'flex', gap: 17, alignItems: 'flex-start' }}><span style={{ fontFamily: 'var(--mono)', color: i % 2 ? 'var(--cyan)' : 'var(--magenta)', minWidth: 30 }}>{number}</span><div><h3>{title}</h3><p>{text}</p></div></article></Reveal>)}</div>
      </div></section>
