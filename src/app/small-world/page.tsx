...
const PLAYERS = [
  ...
  ['ZAKα / GCN2 / PERK / PKR / HRI', 'Stress sensors connecting ribosome or cellular perturbation to the integrated stress response.'],
...
const SOURCES = [
  ...
  ['Structure and thermodynamics', 'RNA secondary/tertiary prediction, resolved structures and simulation-derived features.'],
  ['Experimental outcomes', 'Literature-curated assays and future KritRNA wet-lab results stored with provenance.'],
];
  ...
  return (
    <>
      <section style={{ borderTop: 'none' }}><div className="wrap">
        <Reveal><div className="eyebrow" style={{ marginBottom: 22 }}>11 — Translation small-world engine</div></Reveal>
      ...
      <section className="ink-sec"><div className="wrap">
        <Reveal><div className="sec-head"><div className="eyebrow">Biological layers</div><h2 className="h2">From transcript birth to protein fate</h2><p style={{ color: 'rgba(244,234,213,.7)' }}>The model is organised as connected layers rather than a flat feature list. A perturbation at the stop codon can propagate upstream and downstream through the translation system.</p></div></Reveal>
        <div className="grid cols-4" style={{ background: 'rgba(244,234,213,.14)', borderColor: 'rgba(244,234,213,.14)' }}>{LAYERS.map(([title, text], i) => <Reveal key={title} delay={(i % 4) * 0.04}><article className="cell" style={{ height: '100%', background: i % 2 ? '#141210' : '#10100f' }}><div className="k" style={{ color: i % 4 === 0 ? 'var(--magenta)' : i % 4 === 1 ? 'var(--cyan)' : i % 4 === 2 ? 'var(--forest)' : 'var(--violet)' }}>{String(i + 1).padStart(2, '0')}</div><h3 style={{ color: 'var(--cream)' }}>{title}</h3><p style={{ color: 'rgba(244,234,213,.7)' }}>{text}</p></article></Reveal>)}</div>
      </div></section>

      <section><div className="wrap">
        <Reveal><div className="sec-head"><div className="eyebrow">Molecular network</div><h2 className="h2">The candidate is only one node</h2><p>Each molecular player creates a possible bottleneck, source of competition or safety signal. The network model is intended to make those dependencies explicit.</p></div></Reveal>
        <div className="grid cols-2">{PLAYERS.map(([title, text], i) => <Reveal key={title} delay={(i % 4) * 0.03}><article className="cell" style={{ height: '100%', display: 'flex', gap: 15, alignItems: 'flex-start' }}><span style={{ width: 9, height: 9, borderRadius: '50%', background: i % 3 === 0 ? 'var(--magenta)' : i % 3 === 1 ? 'var(--cyan)' : 'var(--forest)', marginTop: 7, flex: 'none' }} /><div><h3>{title}</h3><p>{text}</p></div></article></Reveal>)}</div>
      </div></section>

      <section><div className="wrap">
        <Reveal><div className="sec-head"><div className="eyebrow">Computational backbone</div><h2 className="h2">Different biological questions need different mathematics</h2><p>No single model class can credibly represent sequence design, ribosome traffic, network diffusion and stress dynamics. The architecture combines methods according to the mechanism being modelled.</p></div></Reveal>
        <div className="grid cols-3">{MATH.map(([title, text], i) => <Reveal key={title} delay={(i % 3) * 0.05}><article className="cell" style={{ height: '100%', borderTop: `4px solid ${i % 3 === 0 ? 'var(--magenta)' : i % 3 === 1 ? 'var(--cyan)' : 'var(--forest)'}` }}><h3>{title}</h3><p>{text}</p></article></Reveal>)}</div>
      ...
      <section className="ink-sec"><div className="wrap">
        <Reveal><div className="sec-head"><div className="eyebrow">Data architecture</div><h2 className="h2">Public knowledge becomes useful only after biological alignment</h2><p style={{ color: 'rgba(244,234,213,.7)' }}>Databases, structures, expression records and experiments describe different parts of the system. The platform is intended to connect them through explicit biological entities and provenance.</p></div></Reveal>
        <div className="grid cols-4" style={{ background: 'rgba(244,234,213,.14)', borderColor: 'rgba(244,234,213,.14)' }}>{SOURCES.map(([title, text], i) => <Reveal key={title} delay={(i % 4) * 0.04}><article className="cell" style={{ height: '100%', background: i % 2 ? '#141210' : '#10100f' }}><div className="k" style={{ color: i % 4 === 0 ? 'var(--magenta)' : i % 4 === 1 ? 'var(--cyan)' : i % 4 === 2 ? 'var(--forest)' : 'var(--violet)' }}>{String(i + 1).padStart(2, '0')}</div><h3 style={{ color: 'var(--cream)' }}>{title}</h3><p style={{ color: 'rgba(244,234,213,.7)' }}>{text}</p></article></Reveal>)}</div>
      </div></section>

      <section><div className="wrap split-2" style={{ display: 'grid', gap: 44, alignItems: 'start' }}>
        <Reveal><div><div className="eyebrow">Dataset philosophy</div><h2 className="h2" style={{ marginTop: 14 }}>Schema before scale</h2><p className="lead" style={{ marginTop: 20 }}>A dual-luciferase readthrough value, Western-blot densitometry and functional protein rescue may describe related biology, but they are not the same label. Pooling them naively produces noise that looks like signal.</p><div className="callout" style={{ marginTop: 24 }}>KritRNA’s dataset philosophy is to preserve assay context, separate outcome types and record uncertainty before increasing dataset size.</div></div></Reveal>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: 'var(--rule)', border: '1px solid var(--rule)', borderRadius: 8, overflow: 'hidden' }}>{SCHEMA.map(([title, text], i) => <Reveal key={title} delay={i * 0.04}><div style={{ background: 'var(--paper)', padding: '19px 22px', display: 'grid', gridTemplateColumns: 'minmax(130px,.65fr) 1.35fr', gap: 18 }}><div style={{ fontFamily: 'var(--mono)', fontSize: '.72rem', color: i % 2 ? 'var(--cyan)' : 'var(--magenta)', textTransform: 'uppercase', letterSpacing: '.05em' }}>{title}</div><div style={{ color: 'var(--ink-soft)', fontSize: '.93rem' }}>{text}</div></div></Reveal>)}</div>
      </div></section>

      <section className="center"><div className="wrap"><Reveal><h2 className="h2">Model the translation system, then challenge it at the bench.</h2></Reveal><Reveal delay={0.06}><p className="lead" style={{ margin: '16px auto 0' }}>The small-world engine is designed to generate testable system-level hypotheses. Experimental evidence remains the authority.</p></Reveal><Reveal delay={0.1}><div style={{ marginTop: 28 }}><Link className="btn btn-solid" href="/pipeline">See the initial research programs →</Link></div></Reveal></div></section>
    </>
  );
}
